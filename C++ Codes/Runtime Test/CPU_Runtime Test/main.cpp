...
class Solve_4Points {
	...
	long long int num_randomSet = 10000000;
...
void Solve_4Points::run(std::vector<cv::Point2f> pts_src, std::vector<cv::Point2f> pts_tar) {
	...
	double src_list_d[8], tar_list_d[8];
	for (int i = 0; i < 4; i++)
	{
		...
		src_list_d[i * 2] = (double)pts_4_src[i].x;
		src_list_d[i * 2 + 1] = (double)pts_4_src[i].y;
		tar_list_d[i * 2] = (double)pts_4_tar[i].x;
		tar_list_d[i * 2 + 1] = (double)pts_4_tar[i].y;
	}
	...
	timeCalculator.start();
	for (int k = 0; k < cycleTimes; k++) {
		runKernel_GPT(pts_4_src, pts_4_tar, GPT_H);
	}
	...
	timeCalculator.start();
	for (int k = 0; k < cycleTimes; k++) {
		runKernel_HO(pts_4_src, pts_4_tar, HO_H);
	}
	...
	timeCalculator.start();
	for (int k = 0; k < cycleTimes; k++) {
		runKernel_GE(src_list, tar_list, H_GE);
	}
	...
	timeCalculator.start();
	for (int k = 0; k < cycleTimes; k++) {
		sks::runKernel_ACA_double(src_list_d, tar_list_d, H_ACA_double);
	}
	...
	timeCalculator.start();
	for (int k = 0; k < cycleTimes; k++) {
		sks::runKernel_SKS_double(src_list_d, tar_list_d, H_SKS_double);
	}
