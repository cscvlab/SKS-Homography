...
void cal_GE(int numsofH, double* d_src_out, double* d_tar_out) {
	double* H_GE_d;
	double* H_GE_h = (double*)malloc(9 * numsofH * sizeof(double));
	CHECK(cudaMalloc((void**)&H_GE_d, 9 * numsofH * sizeof(double)));
	...
	dim3 block_cal_GE = 32;
	dim3 grid_cal_GE = (numsofH + block_cal_GE.x - 1) / block_cal_GE.x;
	int loops = 1;


	cudaEventRecord(start, 0);
	for (int i = 0; i < loops; i++)
		cal_Homo_GE << <grid_cal_GE, block_cal_GE >> > (d_src_out, d_tar_out, H_GE_d, numsofH);
	cudaEventRecord(stop, 0);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&elapsedTime_ref, start, stop);
	loops = 10000 / elapsedTime_ref;
	...
	printf("cal_Homo_GE  Execution configuration<<<%d,%d>>> Time elapsed %f ms\n\n", grid_cal_GE.x, block_cal_GE.x, elapsedTime / loops);
	CHECK(cudaMemcpy(H_GE_h, H_GE_d, 9 * numsofH * sizeof(double), cudaMemcpyDeviceToHost));
	cout << "----------------------------------------------------------" << endl;

	free(H_GE_h);
	CHECK(cudaFree(H_GE_d));
